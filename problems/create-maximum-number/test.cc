...
TEST(CreateMaximumNumberTest, LargeInput1) {
  Solution sol;

  int n1[] = {6,3,1,7,6,6,1,4,7,8,4,1,4,6,1,0,8,9,6,2,3,1,5,4,9,5,4,2,1,7,7,1,4,0,6,2,8,6,2,4,9,8,5,5,5,1,3,5,4,2,3,8,4,1,1,1,0,9,6,7,2,3,8,9,0,3,3,4,6,3,7,7,0,7,9,7,2,8,8,9,8,0,8,2,1,9,8,0,8,4};
  std::vector<int> nums1( std::begin(n1), std::end(n1) );

  int n2[] = {6,4,1,5,0,8,7,6,3,2,7,7,4,1,1,5,3,5,5,9,2,2,0,8,0,5,7,3,9,9,1,2,2,4,2,7,4,5,1,5,6,4,7,5,5,0,0,9,7,3,4,2,3,1,6,8,9,8,3,7,2,8,5,8,5,4,4,7,6,8,1,0,0,5,7,9,5,1,6,8,9,7,8,6,8,6,7,5,2,7};
      std::vector<int> nums2( std::begin(n2), std::end(n2) );

  int k = 90;
  int expected[] = {9,9,9,9,9,9,9,9,8,8,8,7,7,7,5,4,3,5,5,9,2,2,0,8,0,5,7,3,9,9,1,2,2,4,2,7,4,5,1,5,6,4,7,5,5,0,0,9,7,3,4,2,3,1,6,8,9,8,3,7,2,8,5,8,5,4,4,7,6,8,1,0,0,5,7,9,5,1,6,8,9,7,8,6,8,6,7,5,2,7};

  EXPECT_THAT(sol.maxNumber(nums1, nums2, k), testing::ElementsAreArray(expected));
}

TEST(CreateMaximumNumberTest, LargeInput2) {
  Solution sol;
